...
public class ClientboundTierSyncerPacket extends ClientboundUnionPacket {
	...
	// UnionLib's dispatcher calls handleOnClient(LocalPlayer) then runOnClient(Player) when a
	// clientbound packet is received. This is constructed and sent from server-side code (main
	// source set), so it overrides runOnClient(Player) instead of handleOnClient(LocalPlayer):
	// LocalPlayer is a client-only type and this project keeps "main"/"client" strictly split.
	@Override
